...
void thisCin(double *inZiMatrix, double *inZnMatrix, double inEnergy, int xrlintLambda, double *zArray, double *wArray, double *theta1, double theta2, double *out, mwSize n, mwSize m, mwSize nt, mwSize mt, double mT)
{
    ...
    for(ii = 0; ii < nt; ii++){
        for(jj = 0; jj < mt; jj++){
            out[ii+jj*nt] = CinThin(iElement, nElement, inEnergy, lambda, compound, theta1[ii+jj*nt], theta2, mT);                  
        }
    }
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    mT = mxGetScalar(prhs[8]);

    plhs[0] = mxCreateDoubleMatrix((mwSize)theta1MatrixRows,(mwSize)theta1MatrixCol,mxREAL);
    outMatrix = mxGetPr(plhs[0]);

    thisCin(inZiMatrix, inZnMatrix, incidentEnergy, lambdaXrlint, inZMatrix, inWMatrix, theta1Matrix, theta2, outMatrix, (mwSize)inZMatrixRows, (mwSize)inZMatrixCol,(mwSize)theta1MatrixRows, (mwSize)theta1MatrixCol, mT);
